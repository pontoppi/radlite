...
npop        = REFORM(npop_in,nlevels,np)
...
FOR h=0,np-1 DO BEGIN
   ...
   P[h*nlevels:(h+1)*nlevels-1] = TRANSPOSE(R_arr[*,*,h])##REFORM(npop[*,h]) - TOTAL(R_arr[*,*,h],1)*npop[*,h]                   
   ;
   ;Replacing the last equilibrium
   ;equation with mass conservation to
   ;form a linearly independent system
   P[(h+1)*nlevels-1]   = abun_col[h] * rhogas_col[h] - TOTAL(npop[*,h])
ENDFOR
