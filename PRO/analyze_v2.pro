...
;----------------------------------------------------------------------
;                       Read the abundance of water
;----------------------------------------------------------------------

function read_abundance,dat=dat,ext=ext
grid = read_grid()
;
filebase='abundance'
if n_elements(ext) gt 0 then begin
...
readf,1,sizer,sizet,imirt
sizetbig=sizet*(1+imirt)
abundance=dblarr(sizer,sizetbig)
for ir=0,sizer-1 do begin
    for it=0,sizet-1 do begin
        readf,1,a, b
        abundance(ir,it) = a
    ...
    if(imirt eq 1) then begin
        for it=0,sizet-1 do begin
            abundance(ir,sizetbig-it-1) = abundance(ir,it)
        endfor
    endif
endfor
;
...
return,{nr:grid.nr,ntheta:grid.ntheta,nnu:grid.nnu,$
        r:grid.r,theta:grid.theta,nu:grid.nu(1:*),$
        rr:rr,tt:tt,abundance:abundance}
end
...
function read_source_function,ext=ext,all=all
grid = read_grid()
...
filename='scatsource.dat'
openr,1,filename
readf,1,nfr,sizer,sizet,imirt
sizetbig=sizet*(1+imirt)
scat_src=dblarr(sizer,sizetbig,nfr)
...
for inu=1,nfr do begin
   for it=1,sizet do begin
      for ir=1,sizer do begin
         dum = dblarr(1)
         readf,1,dum
         scat_src(ir-1,it-1,inu-1) = dum
         if(imirt eq 1) then begin
            itt = 2*sizet + 1 - it
            scat_src(ir-1,itt-1,inu-1) = dum
         endif
      endfor
   endfor
endfor
