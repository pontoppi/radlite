...
FOR i=0,m.nctrans-1 DO BEGIN
   ...
   R_arr[m.coll_idown[i]-1,m.coll_iup[i]-1,*] = R_arr[m.coll_idown[i]-1,m.coll_iup[i]-1,*] + Cul_arr[i,*]*col.rhogas  ;Rul
   R_arr[m.coll_iup[i]-1,m.coll_idown[i]-1,*] = R_arr[m.coll_iup[i]-1,m.coll_idown[i]-1,*] + Clu_arr[i,*]*col.rhogas  ;Rlu
ENDFOR
...
FOR h=0,m.np-1 DO BEGIN
   ...
   P[m.nlevels-1,h]   = col.abun[h] * col.rhogas[h] - TOTAL(npop[*,h])
ENDFOR
