...
;
; Write the frequency data file
;
openw,1,'frequency.inp'
printf,1,nf
printf,1,' '
for i=0,nf-1 do printf,1,freqnew[i],format='(E13.6)'
close,1
;
...
for ifile=0,nfiles-1 do begin
    ...
    ;;
    file = 'dustopac_'+strcompress(string(ifile+1,format='(I4)'),$
           /remove_all)+'.inp'
    openw,1,file
    printf,1,nf,1
    printf,1,' '
...
;
;----------------------------------------------------------------
;          NOW INCLUDE THE PAH OPACITIES IF REQUESTED
;----------------------------------------------------------------
;
; Check if PAH opacities requested
;
if n_elements(pnc) eq 0 then pnc=[0]
if pnc[0] gt 0 then begin
    ;;
    ;; Set the counter for the dust opacity files
    ;;
    iopac  = ntherm
    ;;
    ;; Loop over all PAHs
    ;;
    for ipah = 1,npah do begin
        ;;
        ;; Increase opacity counter
        ;;
        iopac = iopac + 1
        ;;
        ;; Make the cross-sections using Ruud Visser's program
        ;;
        print,'Making PAH opacities...'
        cross2,pnc[ipah-1],pnh[ipah-1],pz[ipah-1],10000,E_ref=E_ref,C_ref=C_ref
        print,'...done'
        ;;
        ;; Remap on new frequency grid
        ;;
        cross = interpol(C_ref,E_ref,hh*freqnew)
        ;;
        ;; Convert from cross-section per C-atom to cross-section per gram dust
        ;;
        kappa = pnc[ipah-1] * cross / ( pnc[ipah-1]*12*mp + pnh[ipah-1]*mp )
        ;;
        ;; Write to dustopac_***.inp
        ;;
        file = 'dustopac_'+strcompress(string(iopac,format='(I4)'),$
                                       /remove_all)+'.inp'
        openw,1,file
        printf,1,nf,1
        printf,1,' '
        for inu=0,nf-1 do printf,1,kappa[inu]
        printf,1,' '
        for inu=0,nf-1 do printf,1,0.d0
        close,1
    endfor
endif else begin
    npah = 0
...
printf,1,'============================================================================'
;
; Now link the thermal grains
;
for iopac = 1,ntherm do begin
    printf,1,'-1              Way in which this dust species is read (-1=file)'
...
;
; Now link the quantum-heated grains
;
for iopac = ntherm+1,nopac do begin
    printf,1,'-1              Way in which this dust species is read (-1=file)'
    printf,1,'2               2=Quantum-heated grain'
    printf,1,strcompress(string(pnc[iopac-ntherm-1],format='(E13.6)'),$
         /remove_all)+'    Number of C-atoms of PAH/smallcarbgrain'
    printf,1,strcompress(string(iopac,format='(I4)'),$
         /remove_all)+'            Extension of name of dustopac_***.inp file'
    printf,1,'----------------------------------------------------------------------------'
endfor
